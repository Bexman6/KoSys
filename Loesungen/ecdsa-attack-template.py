...
def compute_d(curve, r1, s1, h1, r2, s2, h2, hashfunc):
    ...
    # Calculate the multiplicative inverse of h1 - h2
    h_inv = pow((h1 - h2), -1, order);

    # Calculate the multiplicative inverse of k 
    k_inv = (s1-s2) * h_inv;

    # Restore the original k
    k = pow(k_inv, -1, curve.order)

    # Restore the private key d
...
'''
    ...
    print(public_verification_key.pubkey.order, public_verification_key.curve.order)
